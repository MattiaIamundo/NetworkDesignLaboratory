...
//		final int W = 10;
		final int W = Integer.parseInt (algorithmParameters.get ("Wavelength"));
		...
		for (Demand d : netPlan.getDemands ())
		{
			...
			for(int w=0;w<W;w++) {
				...
				op.addConstraint("sum(r_cnw(c, n, w)) == v_cw(c, w)");
			}
		...
		for (Link e : netPlan.getLinks ())
		{
			...
			op.addConstraint("sum(r_cnw(all, R_lk, all)) <= linkcapacity");
		}
	...
	public List<Triple<String, String, String>> getParameters()
	{
		...
		param.add(Triple.of("Wavelength", "10", "The number of wavelength per fiber"));
		return param;
	}
}
